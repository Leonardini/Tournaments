...
ok<-0
for(cc in C){ pq<-as.bigq(cc$alpha); x<-as.bigq(cc$x); y<-as.bigq(cc$y); E<-length(cc$af); cuts<-cc$cuts
  ...
  if(P&&Dl&&fam) ok<-ok+1 else cat(sprintf("  FAIL idx=%d %s  primal=%s dual=%s family=%s\n",cc$idx,cc$alpha,P,Dl,fam))
}
